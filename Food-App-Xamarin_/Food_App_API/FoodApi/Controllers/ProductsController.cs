...
namespace FoodApi.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        [HttpGet]
        public IActionResult Get([FromQuery] ProductSpecParams productParams)
        {
            var products = (from v in _dbContext.Products
                            where v.Name.Contains(productParams.Search ?? "")|| v.Detail.Contains(productParams.Search ?? "")
                            select v).Take(12).ToList();
            if (!string.IsNullOrEmpty(productParams.SortSelected))
            {
                switch (productParams.SortSelected)
                {
                    case "priceAsc":
        ...
        public IActionResult ProductsByCategory(int categoryId, [FromQuery] ProductSpecParams productParams)
        {
            var products = (from v in _dbContext.Products
                           where v.CategoryId == categoryId
                            && (v.Name.Contains(productParams.Search ?? "") || v.Detail.Contains(productParams.Search ?? ""))
                            select new
                           {
                               Id = v.Id,
                               Name = v.Name,
