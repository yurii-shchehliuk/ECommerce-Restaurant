    ...
    public static class ApiService
    {
        ...
        public static async Task<bool> Login(string email, string password)
        {
            ...
            try
            {
                ...
                //content.Headers.Add("Accept-Encoding", "identity"); //I added it.
            ...
            catch (Exception ex)
            {
                ...
                Debug.Fail(ex.Message,"API Login");
            }
            return false;
    ...
    public static class TokenValidator
    {
        public static async Task CheckTokenValidity()
        {
            try
            {
                ...
                if (Preferences.ContainsKey("tokenExpirationTime"))
                {
                    var expirationTime2 = Preferences.Get("tokenExpirationTime", 0);
                }
                Preferences.Set("currentTime", UnixTime.GetCurrentTime());
                var currentTime = Preferences.Get("currentTime", 0);
                if (Convert.ToInt32(expirationTime) < currentTime)
                {
